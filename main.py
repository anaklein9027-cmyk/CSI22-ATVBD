...
class Application:
    ...
    def listar(self):
        ...
        try:
            ...
            for widget in self.container12.winfo_children():
            ...
            treeview = ttk.Treeview(columns=( "ID","Nome", "CPF_CNPJ", "Nascimento", "CEP", "Endereço", "Contato"))
            treeview.heading("ID", text="ID")
            treeview.heading("Nome", text="Nome")
            treeview.heading("CPF_CNPJ", text="CPF_CNPJ")
            treeview.heading("Nascimento", text="Nascimento")
            treeview.heading("CEP", text="CEP")            
            ...
            for res in resultados:
                ...
                treeview.insert("", 'end', text =f"L{i}", values =(f"{idd}", f"{nome}" ,f"{cpf_cnpj}", f"{nascimento}", f"{CEP}" , f"{endereço}", f"{contato}"))
                i+=1
...
root.mainloop()from Banco import Banco
from tkinter import *
from Usuarios import Usuarios
import requests
